...
class HighDimVisualizer:
    ...
    def load_df(self, df, class_column, class_names, scaler=StandardScaler()):
        """
        Выделяем из используемого датасета X - признаки нашего датасета и y - нашу целевую переменную

        Parameters:
        -----------
            df (pd.DataFrame): Входной датасет.
            class_column (str): Название колонки с целевой переменной.
            scaler (object, optional): Объект для масштабирования признаков. По умолчанию StandardScaler().
        ...
        self.feature_names = [x for x in df.columns if x != class_column]

        self.X = df[self.feature_names].to_numpy()
        if scaler:
            self.X = scaler.fit_transform(self.X)
        self.y = df[class_column].to_numpy()
    ...
    def plot_pairplot(self, X, y, feature_names, n_features=5):
        ...
        df['Class'] = y
        df['Class'] = df['Class'].map(self.class_names)
    ...
    def plot_pca_2d_3d(self, X, y):
        ...
        axes[0].legend(handles, list(self.class_names.values()),
                      title="Классы")
        ...
        ax3d.legend(handles3d, list(self.class_names.values()),
                    title="Классы")
    ...
    def plot_tsne_visualization(self, X, y):
        ...
        if X.shape[0] > 1000:
            ...
            # Определяем безопасное количество компонент
            max_components = min(50, n_samples, n_features)
            if max_components < 2:
                # Если даже 2 компонента недостижимы, используем исходные данные
                print(f"  Внимание: слишком мало данных (max_components={max_components}), PCA не применяется")
                X_reduced = X
            else:
        ...
        for i, perplexity in enumerate(perplexities):
            ...
            if i == 0:
                handles, labels = scatter.legend_elements()
                axes[i].legend(handles[:5], list(self.class_names.values()), title="Классы", fontsize=8)
        ...
        try:
            ...
            for n_neighbors in n_neighbors_list:
                for min_dist in min_dists:
                    ...
                    handles, labels = scatter.legend_elements()
                    axes[plot_idx].legend(handles[:5], list(self.class_names.values()), title="Классы")
    ...
    def plot_parallel_coordinates(self, X, y, feature_names, n_features=10):
        ...
        legend_elements = [Line2D([0], [0], color=colors_plt[i], lw=2, 
                                 label=list(self.class_names.values())[i])
                          for i, class_val in enumerate(unique_classes)]
    ...
    def plot_radar_chart(self, X, y, feature_names, n_features=6):
        ...
        for idx, class_val in enumerate(unique_classes):
            ...
            # Рисуем радар-чарт
            axes[idx].plot(angles, values, 'o-', linewidth=2, label=list(self.class_names.values())[idx])
            axes[idx].fill(angles, values, alpha=0.25)
            
            # Настройки
            axes[idx].set_xticks(angles[:-1])
            axes[idx].set_xticklabels(selected_features, fontsize=8)
            axes[idx].set_title(list(self.class_names.values())[idx], size=14, y=1.1)
            axes[idx].grid(True)
    ...
    def plot_interactive_3d(self, X, y, feature_names):
        ...
        fig = go.Figure(data=[
            go.Scatter3d(
                ...
                text=[f'Class: {self.class_names[label]}<br>Point: {i}' for i, label in enumerate(y)],
                hoverinfo='text'
            )
        ])
        ...
        for idx, (name, model) in enumerate(methods):
            ...
            try:
                ...
                if name == 't-SNE' and X.shape[0] > 1000:
                    # PCA предобработка для t-SNE на больших данных
                    n_samples, n_features = X.shape
                    max_comp = min(50, n_samples, n_features)
                    if max_comp < 2:
                        print(f"    Предупреждение: max_components={max_comp} < 2, пропускаем PCA")
                        X_reduced = X
                    else:
                        pca_pre = PCA(n_components=max_comp, random_state=self.random_state)
                ...
                handles, labels = scatter.legend_elements()
                axes[idx].legend(handles, list(self.class_names.values()),
                                 title="Классы")
                ...
                # В случае ошибки просто показываем сообщение, не пытаясь рисовать X_emb
    ...
    def run_full_visualization(self):
        ...
        self.plot_pairplot(self.X, self.y, self.feature_names, n_features=8)
        self.plot_correlation_heatmap(self.X, self.feature_names)
        self.plot_pca_2d_3d(self.X, self.y)
        self.plot_tsne_visualization(self.X, self.y)

        # Пробуем UMAP если установлен
        try:
            self.plot_umap_visualization(self.X, self.y)
        except ImportError:
            print("\nПримечание: Для UMAP визуализации установите: pip install umap-learn")

        # Дополнительные визуализации
        self.plot_parallel_coordinates(self.X, self.y, self.feature_names, n_features=8)
        self.plot_radar_chart(self.X, self.y, self.feature_names, n_features=6)

        # Интерактивная визуализация
        self.plot_interactive_3d(self.X, self.y, self.feature_names)

        self.compare_dim_reduction_methods(self.X, self.y)
